...
export const updatePrivacyPolicy: RequestHandler = async (req, res, next) => {
  let id = req.params.id;

  const upd = await privacyPolicyModel.findOne({
    where: {
      id: id,
    },
  });
  ...
  if (id === id) {
    ...
    const upda = await privacyPolicyModel.update(updatePrivacyPolicy, {
      where: {
        id: id,
      },
    });
